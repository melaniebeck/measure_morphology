...
class Galaxy(object):
    ...
    def __init__(self, hdulist, filename, flags):
        ...
        if not np.isnan(self.Rp):
            ...
            [self.r20, self.r20_c], [self.r80, self.r80_c], \
                [self.C, self.C_c] = self.get_concentration(image)
            ...
            self.r20_c, self.r80_c, self.C_c = np.nan, np.nan, np.nan
            self.G, self.G2 = np.nan, np.nan 
            self.G_c, self.G2_c = np.nan, np.nan 
            self.M20, self.M10_c, self.Mx, self.My = np.nan, self.x, self.y
    ...
    def get_asymmetry(self, image, aper):
        '''
        ...
        while True:
            ...
            if ga[0] == np.min(ga):
                ...
                utils.checkdir('output/asymimgs/')
    ...
    def get_concentration(self, image):
        ...
        # (Following Lotz 2004)
        r20, r80 = [], []
        # Rp --> elliptical; Rp_c --> circular
        for rp, ax, ay in zip([self.Rp, self.Rp_c], [self.Ax, self.Ax_c], 
                              [self.Ay, self.Ay_c]): 

            tot_aper = CircularAperture((ax, ay), 1.5*rp)
            ...
            if not np.any(np.isnan(interp_ratio)):
                r20.append(utils.get_intersect(interp_ratio, 0.2, interp_radii))
                r80.append(utils.get_intersect(interp_ratio, 0.8, interp_radii))
            else:
                r20.append(np.nan)
                r80.append(np.nan)
    ...
    def get_gini2(self, image):
        ...
        for rp, rp_sb in zip([self.Rp, self.Rp_c], [self.Rp_SB, self.Rp_SB_c]):
            ...
            mask = utils.get_SB_Mask(rp, rp_sb, image, self.name)*image
